...
async fn session_cursor_with_type() {
    ...
    let mut cursor: crate::cursor2::session::SessionCursor<crate::bson::Document> =
        coll.find(doc! {}).session(&mut session).await.unwrap();

    let _ = cursor.next(&mut session).await.unwrap().unwrap();

    let mut cursor_with_type: crate::cursor2::session::SessionCursor<crate::bson::RawDocumentBuf> =
        cursor.with_type();
